...
| 引用准确率(接地) | **94%** (137/145) |
| 拒答准确率 | **93%** (13/14) |
| 幻觉引用 | **4** —— 全部出现在「该拒答」的题上(no_answer 3 + out_of_scope 1) |

按类别:

| 类别 | n | 意图 | 召回 | 答案 | 引用接地 | 拒答 |
|---|---|---|---|---|---|---|
...
> **多文档(一问多文)**是重点也是最难的:在 **54 道多跳题**(每题需 ≥2 篇文档)上 —— 引用接地 **0.98**、该类**幻觉引用 0**、答案正确率 **0.94**,检索召回 **0.70**(口径是「一题所需的**每一段**证据都要命中」的严格 all-of,而非宽松的 any-of recall@k,所以数值天然偏低 —— 与别处的宽松口径不可直接横比)。多跳召回仍是全表最弱项,也是后续要重点啃的方向。

> **已知薄弱点(失败案例,如实记录):** ① 4 条幻觉引用全部出现在**应当拒答**的 no_answer / out_of_scope 题上 —— 即模型偶尔会为「文档里没有」的问题硬凑出处,这是引用校验下一步要堵的口子;② 5 道 out_of_scope 题里有 1 道被误判为 in_scope(意图 0.80,样本小,统计意义有限);③ 拒答准确率 0.89/0.93 不是满分,说明边界题仍有漏判。这些短板连同 35 条检索召回失败已整理进[失败案例库](docs/failure-cases.md),逐条复现修复。
