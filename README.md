...
> **已知薄弱点(失败案例,如实记录):** ① 4 条幻觉引用全部出现在**应当拒答**的 no_answer / out_of_scope 题上 —— 即模型偶尔会为「文档里没有」的问题硬凑出处,这是引用校验下一步要堵的口子;② 5 道 out_of_scope 题里有 1 道被误判为 in_scope(意图 0.80,样本小,统计意义有限);③ 拒答准确率 0.89/0.93 不是满分,说明边界题仍有漏判。这些短板会进失败案例库,逐条复现修复。
