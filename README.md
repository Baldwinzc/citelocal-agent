...
> **多文档(一问多文)**是重点也是最难的:26 道 offline 多跳题(每题需 ≥2 篇文档),**在 106 篇笔记的语料上实测** —— 引用接地 **100%**、**幻觉引用 0**、答案正确率 **96%**,检索召回 **0.64**(它要求同时命中一道题所需的每一段证据,所以召回天然偏低)。语料从 9 篇扩到 106 篇后,这些数字基本不变 —— 混合检索 + 重排在 100+ 干扰文档中仍能稳定召回正确的多篇来源。数字取决于 `LLM_MODEL` 背后的回答模型;改了就重跑 `run_eval`。
