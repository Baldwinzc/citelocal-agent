...
下表是一次**全量实测**:`offline_sample` 的**全部 159 条**,跑在仓库自带的 **106 篇 `sample_notes/`** 语料上,答题 + 评判模型为 **`gpt-5.4-mini`**(经 OpenAI 兼容网关)。数字随模型 / 语料变化 —— 换 `LLM_MODEL` 重跑 `run_eval --split offline_sample` 即可复现。(另有 31 条 `full_corpus` 用例需先下载 8 篇 demo 论文,未计入此表。)
...
| 检索召回(单次检索) | **0.89** |
| 源覆盖(agent 端到端) | **0.95** |
| 答案正确率(LLM 评判) | **97%** (141/145) |
| 引用准确率(接地) | **93%** (135/145) |
| 拒答准确率 | **93%** (13/14) |
| 幻觉引用 | **2**(此次 run;不同 run 在 0–4 间波动,均落在该拒答的题上) |
...
> **多文档(一问多文)**是重点也是最难的:在 **54 道多跳题**(每题需 ≥2 篇文档)上 —— 引用接地 **0.98**、该类**幻觉引用 0**、答案正确率 **0.93**。检索指标分开看:单次检索召回 **0.80**(严格 all-of:一题所需的**每一段**证据都要命中,非宽松 any-of recall@k,不可与别处横比),agent **端到端源覆盖 0.87** —— 编排拆子问题逐个检索,找回了单次检索在复合问题上漏掉的源。这两个数都比早先(0.70 / 0.83)有提升:检索改用**双阈值门控**(`SCORE_THRESHOLD` 仍当拒答闸门,闸门开后用更松的 `SUPPORT_THRESHOLD` 纳入分数略负的第二源),把多跳召回从 0.70 抬到 0.80、**而拒答行为不变**(诊断与做法见[失败案例库](docs/failure-cases.md))。仍 <1.0 的缺口是 reranker 给第二源深负分的那批,要靠更强重排去啃。

> **已知薄弱点(失败案例,如实记录):** ① 幻觉引用都落在**应当拒答**的 no_answer / out_of_scope 题上(不同 run 0–4 条)—— 模型偶尔为「文档里没有」的问题硬凑出处,是引用校验下一步要堵的口子;② 5 道 out_of_scope 题里有 1 道被误判为 in_scope(意图 0.80,样本小);③ 拒答准确率 0.89/0.93 非满分,边界题仍有漏判;④ 多跳端到端覆盖 0.87 未到顶,剩余缺口是 reranker 深负分(双阈值门控已规避「下调阈值伤拒答」的代价)。这些连同 27 条单次检索召回失败及其根因诊断,都整理进[失败案例库](docs/failure-cases.md)。
...
`.env`(见 `.env.example`):`OPENAI_API_KEY`、`LLM_MODEL`(默认 `openai:gpt-4.1`;任意 `init_chat_model` id,含 `ollama:llama3.1`)、`EMBEDDING_MODEL`(`BAAI/bge-small-en-v1.5`)、`RERANKER_MODEL`、`TOP_K`/`CANDIDATE_K`、`SCORE_THRESHOLD`(拒答闸门,已校准,见 `scripts/calibrate_threshold.py`)、`SUPPORT_THRESHOLD`(闸门开后纳入支持性 chunk 的更松阈值,默认 -2.5;设为等于 `SCORE_THRESHOLD` 即关闭)、`CHROMA_PATH`/`CHROMA_COLLECTION`。
