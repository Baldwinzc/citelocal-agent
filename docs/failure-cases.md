...
## 1. 单次检索召回失败(确定性,35 / 159 条 recall < 1.0)

「单次检索召回」在 `TOP_K=4` 下计算,口径是「一题所需的**每一段**证据都要命中」(严格 all-of)。**注意:这衡量的是对整条复合问题做一次检索;agent 实际走编排拆子问题,端到端覆盖高得多(多跳 0.70 → 0.83)。** 下面的「漏检」都是单次检索口径,根因诊断见 1d。
...
- **真正有效但有代价的杠杆 = 下调阈值**:阈值 0.0 → -2.0 时总体召回 0.86→0.91、多跳 0.70→0.78;但「该拒答」题的检索泄漏从 2/14 升到 4/14,直接威胁拒答 / 幻觉(本就是弱项)。原校准注释「[-2.5,2.5] 拒答仍 100%」在 106 篇规模下已不成立。

### 候选修法(按性价比排序,待验证)

- **改进度量已落地**:`run_eval` 现在同时报「单次检索召回」与「agent 端到端源覆盖」——后者(多跳 0.83)才是系统真实表现。单次召回作为检索子系统的诊断指标保留。
- **reranker 是真瓶颈**:换更强 / 领域适配的 cross-encoder,或对多跳子问题用更贴合的查询去重排(编排已在拆子问题,可让重排只面对单跳子问题而非复合问题)。
- **阈值按场景分档**:对已判 in_scope 的检索用更松的阈值(救浅负簇),拒答判定仍用严阈值——需重跑 `calibrate_threshold.py` 并复核拒答泄漏。
- 1b 单源漏检:查询改写 / 关键实体加权,让该文档的 chunk 在重排里抬到阈值之上。
