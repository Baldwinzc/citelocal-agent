...
## 1. 单次检索召回失败(确定性;演进:35 → 27 → 更少)

「单次检索召回」在 `TOP_K=4` 下计算,口径是「一题所需的**每一段**证据都要命中」(严格 all-of)。**注意:这衡量的是对整条复合问题做一次检索;agent 实际走编排拆子问题,端到端覆盖高得多(现多跳 0.92 / 总体 0.97)。** 下面 1a–1c 的清单是**最初**(ms-marco 单阈值 `SCORE_THRESHOLD=0.0`)的 **35 条**漏检,保留作根因记录。两步改进后已大幅缩小:① 双阈值门控(1d)救回浅负簇 → 27 条、多跳召回 0.70→0.80;② 换 reranker 为 `bge-reranker-v2-m3`(见 1d)→ 多跳召回 **0.89**、总体单次召回 **0.94**。
...
- **reranker 已升级(已落地)**:经[选型实验](reranker-selection.md)(`scripts/rerank_bakeoff.py`)定型 **`bge-reranker-v2-m3`** + 重校准阈值(gate 0.2 / support 0.0)。全量验收:多跳单次召回 0.80→**0.89**、端到端覆盖 0.87→**0.92**、答案 97→99%,**拒答不变**。代价是 CPU 延迟(列为后期优化项)。仍漏的极少数是 reranker 也给深负分的题——下一步可让重排只面对编排拆出的**单跳子问题**而非复合问题。
